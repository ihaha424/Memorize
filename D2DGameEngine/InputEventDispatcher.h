...
template<>
class EventDispatcher<InputEvent> : public IEventDispatcher {
	LOG_REGISTER_OBJ(EventDispatcher<InputEvent>)
...
public:
	...
	void Dispatch(IEvent* e) override {
		if (!e || GetStaticEventTypeID() != e->GetEventTypeID()) return;

		_locked = true;

		InputEvent* inputEvent = static_cast<InputEvent*>(e);
		OBJ_MESSAGE(dbg::text("Received an input event!"));
	...
	EventTypeID GetStaticEventTypeID() const override {
		static const EventTypeID STATIC_EVENT_TYPE_ID{
			InputEvent::STATIC_EVENT_TYPE_ID
		};
